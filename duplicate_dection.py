...
def generate_image_hashes(image_files):
    hashes = {}
...
def extract_video_hashes(video_files):
    video_hashes = {}
...
def group_similar_hashes(hashes, threshold):
    ...
    for bucket_files in hash_buckets.values():
        ...
        for i in range(n):
            ...
            if len(group) > 1:
                ...
                similar_groups.append({'files': group, 'similarity': round(similarity, 2)})
...
def main():
    ...
    if not args.no_images:
        ...
        if image_files:
            image_hashes = generate_image_hashes(image_files)
            print("Grouping similar images...")
            image_groups = group_similar_hashes(image_hashes, args.threshold)
        else:
    ...
    if not args.no_videos:
        ...
        if video_files:
            video_hashes = extract_video_hashes(video_files)
            print("Grouping similar videos...")
            video_groups = group_similar_hashes(video_hashes, args.threshold)
        else:
